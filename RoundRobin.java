...
@Route(value = "Page8")
public class RoundRobin extends VerticalLayout{
	...
	public RoundRobin(@Autowired GreetService service) {
		H3 sign = new H3 (" VS ");
		...
		H4 message = new H4("Here are all the matches:");
		
		// nested loop to calculate pairs
		for(int i = 0; i< profiles.size();i++){
			for(int j = i; j< (profiles.size());j++) {
				
				//if i and j are same, don't match as a pair
				if(i != j) {
				slot1.add(profiles.get(i));
				slot2.add(profiles.get(j));
				}
			}
		}
